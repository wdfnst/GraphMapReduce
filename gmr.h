...
public:
    /* 根据不同的算法, 对图进行初始化 */
    virtual void initGraph(GRAPH_DATA *graph) = 0;
...
/* 将图中指定id的顶点的值进行更新, 并返回迭代是否结束 */
void updateGraph(GRAPH_DATA *graph, std::list<KV> &reduceResult, UpdateMode upmode) {
    int i = 0;
    float currentMaxDeviation = 0.0;
    auto iter = reduceResult.begin();
    /* 将reduceResult中id与graph中vertex.id相同key值更新到vertex.value */
    while (i < graph->numMyVertices && iter != reduceResult.end() ) {
        if (iter->key > graph->vertexGID[i]) i++;
        else if (graph->vertexGID[i] > iter->key) iter++;
        else {
            /* 计算误差, 并和老值进行比较, 判断迭代是否结束 */
            float deviation = fabs(iter->value - graph->fvwgts[i]);
            /* 与老值进行比较, 如果变化小于threhold,则将vertex.status设置为inactive */
...
/*将单个节点内的顶点映射为Key/value, 对Key排序后，再进行规约*/
void computing(int rank, GRAPH_DATA *graph, char *rb, int recvbuffersize, GMR *gmr) {
    std::list<KV> kvs;
    Vertex vertex;

    recordTick("bgraphmap");
    for (int i=0; i<graph->numMyVertices; i++) {
        vertex.id = graph->vertexGID[i];
        vertex.value = graph->fvwgts[i];
        vertex.neighborSize = graph->nborIndex[i+1] - graph->nborIndex[i];
        if (vertex.neighborSize > MAX_NEIGHBORSIZE) {
            perror("Neighbor Size exceeds the maximum neighbor size.\n");
            exit(0);
        }
        int neighbor_sn = 0;
        for (int j=graph->nborIndex[i]; j<graph->nborIndex[i+1]; j++, neighbor_sn++) {
//             printf("Process %d to here:%d-->%d\n", rank, vertex.id, graph->nborGID[j]);
            vertex.neighbors[neighbor_sn] = graph->nborGID[j];
            /* 将边的权重从图中顶点拷贝到vertex.edgewgt[k++]中 */
            vertex.edgewgt[neighbor_sn] = graph->fadjwgt[j];
        }
        gmr->map(vertex, kvs);
    ...
    recordTick("brecvbuffermap");
    for (int i = 0 ; i < recvbuffersize; ) {
//         printf("to here: get into the loop of recvbuffer.\n");
        int vid, eid, location, eloc, edgenum = 0;
        float fvwgt, fewgt;
        memcpy(&vid, rb + i, sizeof(int));
        memcpy(&location, rb + (i += sizeof(int)), sizeof(int));
...
void printTimeConsume(int rank) {
    printf("Process %d 迭代次数:%d(%-6.2f%%), 迭代残余误差:%ef, 本次迭代耗时:"
            "%f=(%f[exdata] + %f[map] + %f[sort] + %f[reduce] + %f[updategraph] + %f"
